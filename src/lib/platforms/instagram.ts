    ...
    try {
      if (context.type === "message") {
        if (!context.recipientPlatformId) {
          throw new Error("recipientPlatformId required for Instagram DM");
        }
        const url = `https://graph.facebook.com/${this.apiVersion}/${this.instagramBusinessAccountId}/messages`;
        const response = await fetch(url, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            recipient: { id: context.recipientPlatformId },
            message: { text },
            access_token: this.accessToken,
          }),
        });
        const data = await response.json();
        if (data.error) throw new Error(data.error.message);
        return { platformMessageId: data.message_id || "", status: "sent" };
      }

      if (!context.parentPlatformMessageId) {
        throw new Error(
          "parentPlatformMessageId required for IG comment reply",
        );
      }
      const url = `https://graph.facebook.com/${this.apiVersion}/${context.parentPlatformMessageId}/replies`;
      const response = await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await response.json();
      if (data.error) throw new Error(data.error.message);
      return { platformMessageId: data.id || "", status: "sent" };
    } catch (error) {
      return {
        platformMessageId: "",
        status: "failed",
