    ...
    try {
      if (context.type === "message") {
        if (!context.recipientPlatformId) {
          throw new Error("recipientPlatformId is required for Messenger DM");
        }
      ...
      // Comment / mention reply: POST to the parent comment's /comments edge
      if (!context.parentPlatformMessageId) {
        throw new Error(
          "parentPlatformMessageId is required for comment reply",
        );
      }
      const url = `https://graph.facebook.com/${this.apiVersion}/${context.parentPlatformMessageId}/comments`;
      const response = await fetch(url, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
