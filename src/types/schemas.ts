...
export const sendOtpSchema = z.object({
  phone: z
    .string()
    .regex(/^\+91[6-9]\d{9}$/, 'Invalid Indian phone number. Must be +91 followed by 10 digits starting with 6-9'),
});
...
export const createOrgSchema = z.object({
  ...
  team_size: z.enum(['1', '2-5', '6-10', '11-25', '26-50', '51-100', '100+']).optional(),
...
export const createPostSchema = z.object({
  ...
  content_json: z.object({
    ...
    platform_overrides: z.record(z.object({
...
export const generateContentSchema = z.object({
  ...
  platform: z.enum(['instagram', 'facebook', 'twitter', 'linkedin', 'whatsapp', 'sharechat', 'youtube']),
  tone: z.enum(['professional', 'casual', 'humorous', 'festive', 'hinglish', 'formal_hindi', 'regional_casual']).default('professional'),
  language: z.enum(['en', 'hi', 'ta', 'te', 'bn', 'mr', 'gu', 'kn', 'ml', 'pa']).default('en'),
...
export const translateContentSchema = z.object({
  ...
  source_language: z.enum(['en', 'hi', 'ta', 'te', 'bn', 'mr', 'gu', 'kn', 'ml', 'pa']),
  target_language: z.enum(['en', 'hi', 'ta', 'te', 'bn', 'mr', 'gu', 'kn', 'ml', 'pa']),
});
...
export const suggestRepliesSchema = z.object({
  ...
  language: z.enum(['en', 'hi', 'ta', 'te', 'bn', 'mr']).default('en'),
  tone: z.enum(['professional', 'casual', 'friendly', 'hinglish']).default('professional'),
...
export const checkoutSchema = z.object({
  ...
  billing_state: z.string().min(1, 'Billing state is required for GST calculation'),
  gst_number: z
    .string()
...
export const createListeningQuerySchema = z.object({
  name: z.string().min(1).max(255),
  keywords: z.array(z.string().max(100)).min(1, 'At least one keyword required'),
  excluded_keywords: z.array(z.string().max(100)).default([]),
  platforms: z.array(z.enum(['twitter', 'instagram', 'facebook', 'youtube'])).default(['twitter', 'instagram']),
  languages: z.array(z.enum(['en', 'hi', 'ta', 'te', 'bn', 'mr'])).default(['en', 'hi']),
});
...
export const schemas = {
  auth: { sendOtp: sendOtpSchema, verifyOtp: verifyOtpSchema, register: registerSchema, login: loginSchema },
  org: { create: createOrgSchema, update: updateOrgSchema, invite: inviteMemberSchema, updateRole: updateMemberRoleSchema },
  post: { create: createPostSchema, update: updatePostSchema, schedule: schedulePostSchema, bulk: bulkCreatePostsSchema, approval: postApprovalSchema },
  inbox: { reply: replyMessageSchema, assign: assignConversationSchema, status: updateConversationStatusSchema, tags: addConversationTagsSchema },
  analytics: { dateRange: dateRangeSchema, report: createReportSchema },
  ai: { generate: generateContentSchema, translate: translateContentSchema, hashtags: generateHashtagsSchema, replies: suggestRepliesSchema, sentiment: sentimentAnalysisSchema },
  billing: { checkout: checkoutSchema, updateSubscription: updateSubscriptionSchema },
  media: { update: updateMediaSchema, transform: transformMediaSchema },
  listening: { createQuery: createListeningQuerySchema, createAlert: createAlertSchema },
