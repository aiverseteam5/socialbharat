...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const channelResponse = await fetch(`${channelUrl}&access_token=${accessToken}`)
    ...
    redirect(`/dashboard/settings/social-accounts?error=${encodeURIComponent(error instanceof Error ? error.message : 'Failed to connect YouTube')}`)
  }
}
