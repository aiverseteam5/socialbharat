...
export async function GET(request: NextRequest) {
  ...
  if (!(await verifyState("instagram", state))) {
  ...
  try {
    ...
    redirect(
      `/dashboard/settings/social-accounts?error=${encodeURIComponent(error instanceof Error ? error.message : "Failed to connect Instagram")}`,
    );
  }
}
